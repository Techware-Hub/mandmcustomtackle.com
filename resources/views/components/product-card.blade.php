<article class="reveal group flex h-full flex-col overflow-hidden rounded-lg border border-sky-100 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-sky-300 hover:shadow-xl hover:shadow-sky-100" data-aos="fade-up" data-aos-duration="800">
    <a href="{{ route('products.show', $product->slug) }}" class="block overflow-hidden">
        @php
            $hasVariants = isset($product->variants) && $product->variants->isNotEmpty();
        @endphp
    ...
    <div class="flex flex-1 flex-col p-5">
        <div class="flex items-center justify-between gap-3">
            ...
            <span class="text-lg font-black text-blue-950">{{ $hasVariants ? 'From ' : '' }}${{ number_format((float) $product->price, 2) }}</span>
        </div>
        ...
        <div class="mt-5 grid grid-cols-2 gap-3">
            @if (isset($product->id) && ! $hasVariants)
            ...
            @else
                <a href="{{ route('products.show', $product->slug) }}" class="rounded-lg bg-sky-500 px-4 py-2 text-center text-sm font-bold text-white transition duration-300 hover:scale-[1.02] hover:bg-blue-700">{{ $hasVariants ? 'Choose Options' : 'View Product' }}</a>
            @endif
