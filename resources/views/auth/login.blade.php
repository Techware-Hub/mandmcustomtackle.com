...
<section class="mx-auto max-w-md px-4 py-14 sm:px-6 lg:px-8">
    <div class="rounded-lg border border-sky-100 bg-white p-6 shadow-sm">
        ...
        <p class="mt-3 text-sm leading-6 text-slate-600">Sign in to view your orders and account details.</p>
        <form method="POST" action="{{ route('login.store') }}" class="mt-6 grid gap-4">
            @csrf
            <label>
                <span class="text-sm font-bold text-slate-700">Email</span>
                <input name="email" type="email" value="{{ old('email') }}" autocomplete="email" class="mt-2 w-full rounded-lg border border-sky-200 px-3 py-3" required autofocus>
            ...
            <label>
                ...
                <input name="password" type="password" autocomplete="current-password" class="mt-2 w-full rounded-lg border border-sky-200 px-3 py-3" required>
                @error('password')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
            </label>
            <label class="flex items-center gap-2 text-sm font-semibold text-slate-700">
                <input type="checkbox" name="remember" value="1" class="rounded border-sky-300 text-sky-600">
                Remember me
            </label>
            <button type="submit" class="rounded-lg bg-blue-950 px-5 py-3 font-bold text-white transition hover:bg-sky-700">Login</button>
        </form>
